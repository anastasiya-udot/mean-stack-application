...
module.exports.changeEmail = function(user, email, host, callback){
    ...
    let userEmailService = require('./../../email/user-email.service.js');
    let sendCallback = require('./../../email/send-email.service.js').sendConfirmChangeEmail;
    let getUserByEmail = require('../../user.service').getUserByEmail;

    let tokenType = "verifyEmailChangeToken";

    getUserByEmail(email, function (existUser){

        if(existUser){
            callback({ "message" : constant.USER_EXIST });
            return;
        }
